...
from quote_pipeline.adapters.kis import KisWsAuthClient
...
class KisWsClient:
    ...
    async def subscribe_domestic_ticks(
        ...
        uri = f"{self.cfg.ws_base_url}/tryitout/HDFSCNT0"
        logger.info(
        ...
        try:
            async with websockets.connect(uri, ping_interval=30) as ws:
                ...
                for exch, symb in items:
                    tr_key = f"D{exch}{symb}"
                    req = self._build_ws_message("HDFSCNT0", tr_key, tr_type="1")
                    await ws.send(req)
                    logger.info("sent register overseas: %s", tr_key)
