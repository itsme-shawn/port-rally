...
from quote_pipeline.clients.kis import (
    KisConfig,
    KisRestAuthClient,
    KisRestClient,
    KisWsAuthClient,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KIS REST/WS 기본 테스트 (토큰/승인키 캐싱)")
    ...
    ws = sub.add_parser("ws", help="WebSocket 실시간 구독 (Approval Key 캐싱 포함)")
...
def main() -> None:
    ...
    ws_auth = KisWsAuthClient(cfg)
    ws_client = KisWsClient(cfg, ws_auth)
    ...
    if args.command == "ws":
        # asyncio.run(ws_client.stream_once("overseas", "NVDA", exch="NAS"))
        asyncio.run(ws_client.stream_once("domestic", "005930"))
        return
