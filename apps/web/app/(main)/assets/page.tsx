...
import { EditAssetButton } from "@/components/asset/EditAssetButton";
...
export default async function AssetDetailPage({ searchParams }: Props) {
    const params = await searchParams;
    const { national, exchange, symbol } = params;

    // 필수 파라미터 검증
    if (!national || !exchange || !symbol) {
        notFound();
    }

    // identifier 생성 (national:exchange:symbol)
    const identifier = `${national}:${exchange}:${symbol}`;

    try {
        // includePrice=true: Redis 캐시에서 현재가 조회 (캐시 없으면 Market-Data API 호출)
        const asset = await getAssetDetails(identifier, true);
            ...
            <div className="container-custom py-10">
                <header className="mb-8">
                    <div className="mb-4 flex items-center justify-between">
                        <BackButton />
                        <EditAssetButton identifier={identifier} />
                    </div>
