...
// OIDC 로그인 시 사용자 조회/생성 처리 (Google 등)
public class CustomOidcUserService implements ReactiveOAuth2UserService<OidcUserRequest, OidcUser> {
    ...
    public Mono<OidcUser> loadUser(OidcUserRequest userRequest) throws OAuth2AuthenticationException {
        return delegate.loadUser(userRequest)
            .flatMap(oidcUser -> {
                ...
                return processOAuthLogin(userInfo, oidcUser);
            });
    }
     ...
     * 3. UserPrincipal 반환
     */
    private Mono<OidcUser> processOAuthLogin(OAuth2UserInfo userInfo, OidcUser oidcUser) {
        ...
        return socialAccountRepository
            ...
            .switchIfEmpty(
                ...
                createNewUser(userInfo, oidcUser)
            )
            .map(principal -> (OidcUser) principal)
            .as(transactionalOperator::transactional);
    }

    private Mono<UserPrincipal> createNewUser(OAuth2UserInfo userInfo, OidcUser oidcUser) {
        ...
        return userRepository.save(newUser)
            .flatMap(savedUser -> {
                ...
                return socialAccountRepository.save(socialAccount)
                    .map(sa -> createUserPrincipal(savedUser, sa, userInfo, oidcUser));
            });
    }
    ...
    private UserPrincipal createUserPrincipal(User user, SocialAccount socialAccount,
            OAuth2UserInfo userInfo, OidcUser oidcUser) {
